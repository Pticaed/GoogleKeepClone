...
export default function NoteCard({ note, onPress, onArchive, onDelete, onPin }: any) {
    ...
    const stop = (fn: () => void) => (e: any) => { e?.stopPropagation?.(); fn(); };

    return (
        <View style={s.wrap} onPointerEnter={() => setHover(true)} onPointerLeave={() => { setHover(false); setShowColors(false); }}>
            <Pressable onPress={onPress}>
                <Surface style={[s.card, { backgroundColor: isD ? c.dbg : c.bg, borderColor: isD ? c.db : c.b }]} elevation={hover ? 4 : 1}>
                    {note.is_pinned && <IconButton icon="pin" size={14} iconColor="#1a73e8" style={s.pin} />}
                    ...
                    <View style={[s.actions, { opacity: hover ? 1 : 0 }]}>
                        {[
                            ...
                            <IconButton key={icon} icon={icon} size={18} onPress={fn} iconColor={color} style={{ margin: 0 }} />
                        ))}
                    </View>
...
const s = StyleSheet.create({
    ...
    card: { padding: 12, borderRadius: 8, borderWidth: 1, minHeight: 100 },
    pin: { position: 'absolute', top: -4, right: -4, zIndex: 1, margin: 0 },
